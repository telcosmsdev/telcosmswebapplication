...
CREATE  TABLE IF NOT EXISTS `telco_sms_db`.`Cliente` (
  ...
  `cliente_referencia` ENUM ('Standard', 'Corporate') NOT NULL ,
...
/*
CREATE TRIGGER init_sms_disponiveis AFTER INSERT ON `telco_sms_db`.`Cliente`
	
 DECLARE @id_cliente INT, @Name nvarchar(30)
 SET @id_cliente = (SELECT id_cliente FROM inserted)
 SET @n_sms_enviadas = 0

 INSERT SmsEnviadas (id_cliente, n_sms_enviadas) VALUES(@id_cliente , n_sms_enviadas)
...
CREATE  TABLE IF NOT EXISTS `telco_sms_db`.`Compra` (
  ...
  `id_cliente` INT NOT NULL ,
  `id_pacote` INT NOT NULL ,
  `referencia_pagamento` VARCHAR(45) NOT NULL ,
